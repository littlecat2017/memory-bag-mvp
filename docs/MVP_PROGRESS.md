...
| M4 自动战斗 | 16% | 基本完成 | 15% | 濒死回退与 faded 状态、Boss 压力的正式淡化效果、战斗演出二次打磨 | 木剑提高攻击；有人等我恢复；空名牌对无名敌人更强；标准 MVP 路线可胜利打过 Boss 并进入结尾 |
...
- 截图中主界面、战斗舞台、背包面板和选择浮层都没有明显遮挡或文本溢出。

### 再下一轮：敌人与 Boss 视觉补强

目标：

- 为普通敌人与忘名猎人补正式立绘或剪影资源。
- 打磨 Boss 战前后的视觉表现，让“忘名猎人”和“空名牌”更符合文档中的关系主题。
- 在不突破每日 50 张限制的前提下，按最少必要数量生成或复用美术资源。

验收：

- 普通战斗和 Boss 战能区分敌人视觉。
- 图片生成日志完整，且不记录任何密钥或临时敏感 URL。
- 新增资源均通过 `verify_art_assets.gd`。
...
- 本日剩余可用额度：50 张；下一轮优先做体验验收，不主动扩大生成量。
...
- 自动战斗已接入基础演出时间线和斩击播放，但仍缺少正式敌人立绘、Boss 专属压迫演出和失败惩罚视觉反馈。
