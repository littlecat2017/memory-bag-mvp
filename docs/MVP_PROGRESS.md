...
- 新增完整通关冒烟脚本 `scripts/debug/verify_full_game_flow.gd`：
  ...
  - Boss 前拒绝献出名字。
  - Boss 战胜利。
  - Boss 后获得“空名牌”时仍遵守 MVP 初始容量 4 的限制，需要在 7x4 可视网格中替换已解锁格的旧记忆。
  - 跑到 `mvp_named_with_reason` 结尾。
- 校准 MVP 自动战斗数值：
  - 降低空壳巡林人攻击压力。
  - 降低忘名猎人生命与攻击压力。
  - “有人等我”的周期恢复从 2 点提高到 4 点。
  - 标准完整路线验收结果：`ending=mvp_named_with_reason hp=21 gold=34`，最终背包保持 4 个已解锁记忆，但 7x4 网格仍完整可见。
- 改造主界面为视觉小说式布局：
  - 全屏背景图作为第一视觉层。
...
- 背包侧栏改为可折叠右侧面板：
  - 使用 image2 生成的背包面板底图。
  - 侧栏记忆卡改为纵向滚动显示，底部操作台改为 7x4 可视网格，避免遮挡底部对话框。
...
- 按 `docs/MVP_TASKS_VFX_AND_BAG_REWORK.md` 完成动效与核心背包交互重构：
  ...
  - 主屏幕下方中心新增常驻记忆背包操作条：左侧垃圾堆、中间 7x4 可视背包网格、右侧新发现。
...
- 根据用户最新视觉方向，将主界面重排为“上半区演出、下半区操作”的结构：
  ...
  - 下半区固定为核心操作台：左侧“弃牌堆”、中央 7x4 “记忆背包”网格、右侧“新记忆”；MVP 初期只解锁 4 格，锁定格灰色可见，强化类似背包棋盘/道具摆放的核心玩法读法。
...
- 根据用户反馈重做底部背包操作台视觉：
  - 使用桌面 image2 PHP 脚本生成 `ui_quick_bag_tray`，左侧弃牌堆、中间 7x4 记忆背包区域、右侧新记忆区均由图片皮肤承载；真实 7x4 交互格由 Godot 按 `data/visual_layout.json` 叠加，不再依赖代码画线作为主视觉。
...
- 2026-06-03 背包布局纠偏：固定显示 7x4 共 28 格记忆背包，MVP 初始只解锁 4 格，其余灰色锁定且不可投放。布局改由 `data/visual_layout.json` 驱动，并新增 `docs/VISUAL_LAYOUT_SPEC.md`、`verify_visual_contract.gd` 验收，避免后续再把容量 4 误实现成 2x2/4 格 UI。
