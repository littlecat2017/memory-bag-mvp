...
- 按 `docs/MVP_TASKS_VFX_AND_BAG_REWORK.md` 逐项处理当前最显眼的问题。
- 先修斩击特效残边、行走绿色回响、9 帧行走/战斗动作流畅度。
- 再把主屏幕下方中心改成常驻记忆背包操作区，并规划左侧垃圾堆、右侧新发现区和拖拽交互。

验收：

- 动效视频中不再出现斩击残边、棋盘格或不明绿色影子。
- 行走和战斗动作至少比当前版本更顺，不出现明显滑步、遮挡或大块特效污染。
- 主屏幕能常驻显示记忆背包格子，并为后续拖拽、垃圾堆、新发现区打好 UI 和数据结构基础。
