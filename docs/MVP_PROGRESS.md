...
| M8 MVP 美术接入 | 4% | 基本完成 | 3% | 立绘透明化、更多角色/敌人/场景资源、特效表实际播放 | 资源路径稳定，图片生成有日志，不超过每日 50 张 |
...
- 在可用 Godot 环境中运行 `scripts/debug/verify_art_assets.gd`。
- 处理立绘白底问题，优先做本地抠图或重新生成透明友好的版本。
- 把 `fx_slash_basic_sheet` 接入战斗演出，而不是只登记资源。
- 用当前 6 张图手动跑一遍序章到森林首战，检查 UI 裁切和遮挡。
...
当前图片生成消耗：6 / 50 张（2026-06-01）

已生成美术规划中建议的 6 张风格测试图：
...
- 本日剩余可用额度：44 张，但下一轮优先做接入和抠图，不主动扩大生成量。
...
- 调试面板功能可用，但视觉很朴素，后续应随正式 UI 一起整理。
- 首批 6 张真实美术资源已接入，但立绘当前是白底 PNG，还不是透明立绘。
- `fx_slash_basic_sheet` 已生成并登记，但尚未接入战斗演出播放。
- 当前 shell 未找到 Godot 可执行文件，Godot headless 验证脚本已准备好，但本轮未能实际运行。
